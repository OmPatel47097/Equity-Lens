...
from services.PortfolioOptimizationMLOpsPipeline import portfolio_optimization_pipeline
                ...
                jsonify(
                    {
                        ...
                        "invalid_symbols": non_valid_symbols,
                    }
                ),
                400,
            )

        optimized_result = portfolio_optimization_pipeline(capital, symbols)
...
def cumulative_return():
    try:
        ...
        portfolio_data_list = [
            stockDataManager.history(ticker=symbol, period=period, interval="1d")[
                ["adj_close"]
            ]
            for symbol in symbols
        ]
        portfolio_data = pd.concat(
            portfolio_data_list, axis=1, keys=[symbol for symbol in symbols]
        )
        ...
        sp500_data = (
            sp500_data.reindex(mean_portfolio_data.index)
            .fillna(method="ffill")
            .fillna(method="bfill")
        )
        ...
        portfolio_cumulative_returns = (
            mean_portfolio_data / mean_portfolio_data.iloc[0] - 1
        ) * 100
        sp500_cumulative_returns = (sp500_data / sp500_data.iloc[0] - 1) * 100

        # Convert dates to strings
        portfolio_cumulative_returns.index = portfolio_cumulative_returns.index.astype(
            str
        )
